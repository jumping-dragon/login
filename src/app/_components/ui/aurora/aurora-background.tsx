...
function resolveInitialPalette(
  ...
  if (preset) {
    const template = BUILT_IN_TEMPLATES.find((t) => t.name === preset) ?? BUILT_IN_TEMPLATES[0]!;
    return {
...
export const AuroraBackground = ({
  ...
  const [stopPositions, setStopPositions] = useState<StopMap>(() => ({ ...init.stopPositions }));
  ...
  const handleSettingsChange = useCallback((partial: Partial<PaletteSettings>) => {
    ...
    if (partial.invertFilter !== undefined) setInvertFilter(partial.invertFilter);
  ...
  return (
    <main className={mainClassName}>
      {showController && (
        <AuroraController settings={paletteSettings} onSettingsChange={handleSettingsChange} />
      )}
      <div
        className={cn(
