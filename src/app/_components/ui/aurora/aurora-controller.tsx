...
export function AuroraController({
	settings,
	onSettingsChange,
}: AuroraControllerProps) {
	const { colors, stopPositions, invertFilter, blendMode, opacity, blur } =
		settings;
	...
	useEffect(() => {
		const onMove = (e: MouseEvent) => {
			...
			const pct = Math.round(
				Math.max(0, Math.min(100, ((e.clientX - left) / width) * 100)),
			);
	...
	const updateColor =
		(key: keyof ColorMap) => (e: React.ChangeEvent<HTMLInputElement>) => {
			onSettingsChange({
	...
	return (
		<div className="fixed top-4 left-4 z-10 w-72 rounded-lg border border-zinc-200 bg-white/90 shadow-lg backdrop-blur dark:border-zinc-700 dark:bg-zinc-800/90">
			<button
				...
				className="flex w-full items-center justify-between gap-4 px-3 py-2 font-medium text-xs text-zinc-600 dark:text-zinc-400"
			>
			...
			{isOpen && (
				<div className="flex flex-col gap-3 border-zinc-200 border-t px-3 py-3 dark:border-zinc-700">
					{/* ── Templates & saved palettes ── */}
					<div className="flex flex-col gap-1.5">
						<span className="font-semibold text-[10px] text-zinc-400 uppercase tracking-wide dark:text-zinc-500">
							Palettes
						</span>
						...
						<div className="flex flex-wrap gap-1.5">
							{BUILT_IN_TEMPLATES.map((t) => (
								<button
									...
									className="flex flex-col items-start gap-0.5 rounded border border-zinc-200 bg-white px-1.5 pt-1 pb-1 shadow-sm transition-shadow hover:shadow-md dark:border-zinc-600 dark:bg-zinc-700"
								>
									<div
							...
							{savedPalettes.map((p) => (
								<div
									key={p.id}
									className="group relative flex flex-col items-start gap-0.5 rounded border border-zinc-200 bg-white px-1.5 pt-1 pb-1 shadow-sm dark:border-zinc-600 dark:bg-zinc-700"
								>
									<button
										type="button"
										title={`Apply "${p.name}"`}
									...
									<button
										...
										className="-right-1.5 -top-1.5 absolute hidden h-4 w-4 items-center justify-center rounded-full bg-zinc-200 text-zinc-500 hover:bg-red-100 hover:text-red-500 group-hover:flex dark:bg-zinc-600 dark:text-zinc-300"
									>
						...
						{showSaveInput ? (
							<div className="flex gap-1.5">
								...
								<button
									...
									className="rounded bg-blue-500 px-2 py-1 font-medium text-[10px] text-white hover:bg-blue-600 disabled:opacity-40"
								>
									Save
								</button>
								<button
								...
								>
									...
									<button
										...
										className={cn(
											...
											activeStop === key
												? "scale-110 ring-blue-400"
												: "ring-black/10",
										)}
										style={{ background: colors[key] }}
									/>
									<div className="-translate-x-1/2 absolute top-6 left-1/2 whitespace-nowrap">
							...
							{OVERLAY_STOPS.map(({ key, label }) => {
								...
								return (
									<label
										...
										title={
											isUnused ? "Unused when invert filter is off" : undefined
										}
									>
					...
					<div className="flex flex-col gap-1.5">
						...
						<label className="flex cursor-pointer items-center gap-2">
							<div
								...
								onClick={() =>
									onSettingsChange({ invertFilter: !invertFilter })
								}
							>
								<div
									className={cn(
							...
							<span className="text-xs text-zinc-600 dark:text-zinc-300">
								{invertFilter
									? "On — colors show as complements"
									: "Off — colors render literally"}
							</span>
						</label>
					</div>
					...
					<div className="flex flex-col gap-1.5">
						...
						<select
							value={blendMode}
							onChange={(e) =>
								onSettingsChange({ blendMode: e.target.value as BlendMode })
							}
					...
					<div className="flex flex-col gap-1.5">
						...
						<input
							...
							onChange={(e) =>
								onSettingsChange({ opacity: Number(e.target.value) / 100 })
							}
					...
					<div className="flex flex-col gap-1.5">
						...
						<input
							...
							value={blur}
							onChange={(e) =>
								onSettingsChange({ blur: Number(e.target.value) })
							}
							className="h-1.5 w-full cursor-pointer accent-blue-500"
						/>
					</div>
